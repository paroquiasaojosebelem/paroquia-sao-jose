...
export default async function Home(){
 ...
 const [{data:masses=[]},{data:live=[]},{data:news=[]},{data:highlights=[]},{data:events=[]},{data:spiritual=[]}]=await Promise.all([
  ...
  s.from('parish_events').select('id,title,category,summary,location,starts_at,ends_at,image_url,featured').eq('active',true).eq('featured',true).gte('starts_at',new Date().toISOString()).order('starts_at').limit(3),
  s.from('spiritual_content').select('id,title,category,summary,author,image_url,published_at').eq('active',true).eq('featured',true).lte('published_at',new Date().toISOString()).order('published_at',{ascending:false}).limit(3)
 ]);
 ...
 return <><Header/>
  ...
  {events.length>0&&<section className="wrap homeAgenda"><div className="sectionHead"><div><span className="eyebrow dark">VIDA PAROQUIAL</span><h2>Próximos eventos</h2></div><Link href="/agenda">Ver agenda completa →</Link></div><div className="homeAgendaGrid">{events.map(e=><article key={e.id} className="homeEventCard">{e.image_url&&<img src={e.image_url} alt=""/>}<div><small>{e.category||'EVENTO'}</small><h3>{e.title}</h3><b>{new Date(e.starts_at).toLocaleDateString('pt-BR',{day:'2-digit',month:'long'})} · {new Date(e.starts_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</b>{e.location&&<p>📍 {e.location}</p>}{e.summary&&<p>{e.summary}</p>}</div></article>)}</div></section>}
  {spiritual.length>0&&<section className="wrap spiritualHome"><div className="sectionHead"><div><span className="eyebrow dark">FÉ E FORMAÇÃO</span><h2>Formação e Espiritualidade</h2></div><Link href="/formacao">Ver todos →</Link></div><div className="spiritualGrid">{spiritual.map(x=><article className="spiritualCard" key={x.id}>{x.image_url&&<img src={x.image_url} alt=""/>}<div><small>{x.category}</small><h2>{x.title}</h2><p>{x.summary}</p><Link href={`/formacao/${x.id}`}>LER CONTEÚDO →</Link></div></article>)}</div></section>}
