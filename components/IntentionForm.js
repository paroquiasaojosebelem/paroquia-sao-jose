...
export default function IntentionForm({action,tipos,metodos,schedule,valor}){
  const today=new Date().toLocaleDateString('en-CA',{timeZone:'America/Belem'});
  ...
  return <form action={action} className="panel formCard"><h2>Enviar uma intenção</h2>
    ...
    <div className="formRow"><label>Data desejada<input type="date" name="mass_date" required min={today} value={date} onChange={e=>setDate(e.target.value)}/></label>
